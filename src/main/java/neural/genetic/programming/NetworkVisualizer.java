...
public class NetworkVisualizer<T> extends Canvas {
    ...
    public void paint(Graphics g) {
        ...
        for (int ii = 0; ii < network.getM(); ii++)// Draw Gates and links between
        {
            for (int jj = 0; jj < network.getN(); jj++) {
                ...
                g.setColor(Color.LIGHT_GRAY);
                neuronList = network.getNeurons()[ii][jj].getInputNeurons();
        ...
        for (int ii = 0; ii < network.getOutputNumber(); ii++)// Draw Output
        {
            ...
            g.setColor(Color.LIGHT_GRAY);
            int gateI = network.getOutput()[ii].getInputNeurons().getFirst().getI();
            int gateJ = network.getOutput()[ii].getInputNeurons().getFirst().getJ();
            if (network.getOutput()[ii].getInputNeurons().getFirst().getJ() == -1) {
        ...
        for (int ii = 0; ii < network.getOutputNumber(); ii++) {
            gateI = network.getOutput()[ii].getInputNeurons().getFirst().getI();
            gateJ = network.getOutput()[ii].getInputNeurons().getFirst().getJ();
            if (network.getOutput()[ii].getInputNeurons().getFirst().getJ() == -1) {
                g.drawLine(a, gateI * dist + 80 + a / 2, 1500, ii * dist + 50 + a + a / 2);
            } else {
                g.drawLine((gateJ + 1) * dist + a, (gateI + 1) * dist + a / 2, 1500, ii * dist + 50 + a + a / 2);
    ...
    public void highlightGate(Graphics2D g, int ii, int jj) {
        ...
        LinkedList<Neuron<T>> neuronList = neuron.getInputNeurons();
